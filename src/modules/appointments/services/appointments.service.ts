...
export class AppointmentsService {
  ...
  async create(
    ...
    const babershops = await this.barbershopRepository.findById(
      createAppointmentDto.barbershop.id,
    );
  ...
  async cancelAppointment(id: number): Promise<Appointment> {
    ...
    return this.repositoty.updateStatus(id, {
  ...
  async updateStatus(
    ...
    if (
      ...
      updateStatusDto.status !== AppointmentStatus.SCHEDULED
    ) {
      throw new BadRequestException(
        'Cannot update status of cancelled appointment',
      );
    }
